...
export function useDialogCheckout(convertedTotal: number, currencyLabel = 'IDR') {
    const { clearCart } = useCartRecoil()
    ...
    const onConfirm = useCallback(() => {
        ...
        clearCart()
        if(paid == convertedTotal - 0.0001)
            toast.success(`Transaksi berhasil. Uangnya pas terima kasih! (${convertCurrency(paid, covertCurrencyLabel)})`)
        else if(paid > convertedTotal - 0.0001) {
            const change = paid - convertedTotal
            toast.success(`Transaksi berhasil. Kembalian ${convertCurrency(change, covertCurrencyLabel)}. Terima kasih!`)
        }
        setOpen(false)
        reset()
        return true
    }, [amount, convertedTotal, clearCart, covertCurrencyLabel, reset])
